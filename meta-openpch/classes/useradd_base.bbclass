...
perform_groupadd () {
    ...
    local group_exists="`grep "^$groupname:" ${rootdir}${sysconfdir}/group || true`"
    if test "x$group_exists" = "x"; then
        local count=0
        while true; do
            eval $PSEUDO groupadd $opts || true
            group_exists="`grep "^$groupname:" ${rootdir}${sysconfdir}/group || true`"
            if test "x$group_exists" = "x"; then
                bbwarn "groupadd command did not succeed. Retrying..."
                sleep 1
            else
                local usergroupdir="${rootdir}${sysconfdir}/user-groups"
                [ ! -d "$usergroupdir" ] && mkdir -p "$usergroupdir"
                local fix_opts=$(echo "$opts" | sed "s@$rootdir@@g")
                echo "groupadd $fix_opts" > "${usergroupdir}/${groupname}.group"
                break
            fi
            count=`expr $count + 1`
...
perform_useradd () {
    ...
    local user_exists="`grep "^$username:" ${rootdir}${sysconfdir}/passwd || true`"
    if test "x$user_exists" = "x"; then
           local count=0
           while true; do
               eval $PSEUDO useradd $opts || true
               user_exists="`grep "^$username:" ${rootdir}${sysconfdir}/passwd || true`"
               if test "x$user_exists" = "x"; then
                   ...
                   sleep 1
                    ...
                    [ ! -d "$usergroupdir" ] && mkdir -p "$usergroupdir"
                    local fix_opts=$(echo "$opts" | sed "s@$rootdir@@g")
                    echo "useradd $fix_opts" > "${usergroupdir}/${username}.user"
                   break
               fi
               count=`expr $count + 1`
               if test $count = $retries; then
...
perform_groupmems () {
    ...
    # groupmems fails if /usr/local/etc/gshadow does not exist
    local gshadow=""
    ...
    local mem_exists="`grep "^$groupname:[^:]*:[^:]*:\([^,]*,\)*$username\(,[^,]*\)*" ${rootdir}${sysconfdir}/group || true`"
    if test "x$mem_exists" = "x"; then
        local count=0
        while true; do
            eval $PSEUDO groupmems $opts || true
            mem_exists="`grep "^$groupname:[^:]*:[^:]*:\([^,]*,\)*$username\(,[^,]*\)*" ${rootdir}${sysconfdir}/group || true`"
            if test "x$mem_exists" = "x"; then
